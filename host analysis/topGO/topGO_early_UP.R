...
genesOfInterest <- read.table("DE_early_UP.txt",header=FALSE)
genesOfInterest <- as.character(genesOfInterest$V1)
geneList <- factor(as.integer(geneUniverse %in% genesOfInterest))
names(geneList) <- geneUniverse

# build the GOdata object in topGO
myGOdata_BP <- new("topGOdata", description="Early Stage UP BP", ontology="BP", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
myGOdata_BP
...
write.table(allRes_BP, file="topGO_early_UP_BP.txt", append=F, quote=F, sep="\t", eol="\n")

# for CC
myGOdata_CC <- new("topGOdata", description="Early Stage UP CC", ontology="CC", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
myGOdata_CC
...
write.table(allRes_CC, file="topGO_early_UP_CC.txt", append=F, quote=F, sep="\t", eol="\n")

#for MF
myGOdata_MF <- new("topGOdata", description="Early Stage UP MF", ontology="MF", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
myGOdata_MF
...
write.table(allRes_MF, file="topGO_early_UP_MF.txt", append=F, quote=F, sep="\t", eol="\n")
