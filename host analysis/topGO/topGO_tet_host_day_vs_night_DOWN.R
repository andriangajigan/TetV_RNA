...
genesOfInterest <- read.table("DE_day_vs_night_DOWN.txt",header=FALSE)
genesOfInterest <- as.character(genesOfInterest$V1)
geneList <- factor(as.integer(geneUniverse %in% genesOfInterest))
names(geneList) <- geneUniverse

# build the GOdata object in topGO
myGOdata_BP <- new("topGOdata", description="Day vs Night DOWN BP", ontology="BP", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
myGOdata_BP
...
write.table(allRes_BP, file="topGO_day_vs_night_DOWN_BP.txt", append=F, quote=F, sep="\t", eol="\n")

# for CC
myGOdata_CC <- new("topGOdata", description="Day vs Night DOWN CC", ontology="CC", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
myGOdata_CC
...
write.table(allRes_CC, file="topGO_day_vs_night_DOWN_CC.txt", append=F, quote=F, sep="\t", eol="\n")

#for MF
myGOdata_MF <- new("topGOdata", description="Day vs Night DOWN MF", ontology="MF", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
myGOdata_MF
...
write.table(allRes_MF, file="topGO_day_vs_night_DOWN_MF.txt", append=F, quote=F, sep="\t", eol="\n")
